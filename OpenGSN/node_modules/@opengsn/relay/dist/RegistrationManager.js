...
class RegistrationManager {
    ...
    managerAddress, workerAddress) {
        this._isSetOwnerCalled = false;
        this._isOwnerSetOnStakeManager = false;
    ...
    async updateLatestRegistrationTxs(hubEventsSinceLastScan) {
        for (const eventData of hubEventsSinceLastScan) {
            switch (eventData.event) {
                case GSNContractsDataTypes_1.RelayServerRegistered:
                    ...
                    break;
                case GSNContractsDataTypes_1.RelayWorkersAdded:
    ...
    async handlePastEvents(hubEventsSinceLastScan, lastScannedBlock, currentBlock, forceRegistration) {
        ...
        const eventNames = [GSNContractsDataTypes_1.HubAuthorized, GSNContractsDataTypes_1.StakeAdded, GSNContractsDataTypes_1.HubUnauthorized, GSNContractsDataTypes_1.StakeUnlocked, GSNContractsDataTypes_1.StakeWithdrawn, GSNContractsDataTypes_1.OwnerSet];
        ...
        for (const eventData of decodedEvents) {
            switch (eventData.event) {
                case GSNContractsDataTypes_1.HubAuthorized:
    ...
    _isRegistrationCorrect() {
        return Utils_2.isRegistrationValid(this.lastMinedRegisterTransaction, this.config, this.managerAddress);
    }
    ...
    async refreshStake() {
        ...
        this._isOwnerSetOnStakeManager = stakeInfo.owner !== Constants_1.constants.ZERO_ADDRESS;
        if (this._isOwnerSetOnStakeManager && !Utils_1.isSameAddress(stakeInfo.owner, this.config.ownerAddress)) {
...
`);
    ...
    async setOwnerInStakeManager(currentBlock) {
        ...
        const details = {
            signer: this.managerAddress,
            gasLimit,
            serverAction: StoredTransaction_1.ServerAction.SET_OWNER,
